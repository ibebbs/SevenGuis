...
namespace CircleDrawer
{
    ...
    {
        private static IEnumerable<Point> GetAreaPoint(TappedRoutedEventArgs args)
        {
            switch (args.OriginalSource)
            {
                case Canvas canvas:
                    var location = args.GetPosition(canvas);
                    yield return new Point(Convert.ToInt32(location.X), Convert.ToInt32(location.Y));
                    break;
            }
        }

        private readonly MainPageViewModel _viewModel;
        private readonly IObservable<Point> _emptyAreaClicked;
